...
class TransferScheduleResource(
  ...
  @Operation(
    summary = "Creates a mapping for a single transfer schedule",
    description = "Creates a mapping for a single transfer schedule. Requires ROLE_NOMIS_MAPPING_API__SYNCHRONISATION__RW",
    requestBody = io.swagger.v3.oas.annotations.parameters.RequestBody(
      content = [Content(mediaType = "application/json", schema = Schema(implementation = TransferScheduleMappingDto::class))],
    ),
    responses = [
      ApiResponse(responseCode = "201", description = "Transfer schedule mapping created"),
      ApiResponse(
        responseCode = "401",
  ...
  @Operation(
    summary = "Gets a mapping for a single transfer schedule by NOMIS event ID",
    description = "Gets a mapping for a single transfer schedule by NOMIS event ID. Requires ROLE_NOMIS_MAPPING_API__SYNCHRONISATION__RW",
    responses = [
      ApiResponse(responseCode = "200", description = "Transfer schedule mapping returned"),
      ApiResponse(
        responseCode = "401",
        description = "Unauthorized to access this endpoint",
        content = [Content(mediaType = "application/json", schema = Schema(implementation = ErrorResponse::class))],
  ...
  @Operation(
    summary = "Gets a mapping for a single transfer schedule by DPS event ID",
    description = "Gets a mapping for a single transfer schedule by DPS event ID. Requires ROLE_NOMIS_MAPPING_API__SYNCHRONISATION__RW",
    responses = [
      ApiResponse(responseCode = "200", description = "Transfer schedule mapping returned"),
      ApiResponse(
        responseCode = "401",
