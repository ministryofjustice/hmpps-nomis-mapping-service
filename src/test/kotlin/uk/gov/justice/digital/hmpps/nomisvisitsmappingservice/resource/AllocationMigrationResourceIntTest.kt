...
class AllocationMigrationResourceIntTest : IntegrationTestBase() {
  ...
  private val ACTIVITY_SCHEDULE_ID = 5555L
  ...
  private fun saveMapping(offset: Int, label: String = MIGRATION_ID) =
    saveMapping(NOMIS_ALLOCATION_ID + offset, ACTIVITY_ALLOCATION_ID + offset, ACTIVITY_SCHEDULE_ID + offset, label)
  ...
  @DisplayName("POST /mapping/allocations/migration")
  @Nested
  inner class CreateMapping {
    ...
    fun `should create mapping`() = runTest {
      ...
      with(saved) {
        ...
        assertThat(activityScheduleId).isEqualTo(ACTIVITY_SCHEDULE_ID)
    ...
    fun `should return bad request if mapping already exists for different Allocation id`() = runTest {
      ...
      postCreateMappingRequest(activityAllocationId = 1)
        ...
        .jsonPath("userMessage").value<String> {
          assertThat(it).contains("Nomis allocation mapping id = $NOMIS_ALLOCATION_ID already exists")
        }
    }

    @Test
    fun `should return created if mapping already exists for same Nomis and DPS id`() = runTest {
      postCreateMappingRequest()
        .expectStatus().isCreated
    ...
    @Test
    fun `should return bad request if allocation already exists for different Nomis allocation id`() = runTest {
      postCreateMappingRequest()
        .expectStatus().isCreated
      ...
      postCreateMappingRequest(nomisAllocationId = 1)
        ...
        .expectBody()
        .jsonPath("userMessage").value<String> {
          assertThat(it).contains("Allocation migration mapping with DPS allocation id=$ACTIVITY_ALLOCATION_ID already exists")
        }
    }

    @Test
    fun `should return conflict if attempting to create duplicate`() = runTest {
      postCreateMappingRequest(activityAllocationId = 101)
        .expectStatus().isCreated
      ...
      postCreateMappingRequest(activityAllocationId = 102)
  ...
  inner class GetMapping {
    ...
    fun `should return OK if mapping exists`() = runTest {
      ...
      webTestClient.get().uri("/mapping/allocations/migration/nomis-allocation-id/$NOMIS_ALLOCATION_ID")
        ...
        .jsonPath("activityScheduleId").isEqualTo(ACTIVITY_SCHEDULE_ID)
  ...
  @DisplayName("GET /mapping/allocations/migration/migrated/latest")
  @Nested
  inner class GetLatestMigratedMapping {

    @Test
    fun `access forbidden when no authority`() {
      webTestClient.get().uri("/mapping/allocations/migration/migrated/latest")
        .exchange()
        .expectStatus().isUnauthorized
    }
  ...
  inner class GetMappingByMigrationId {
    ...
    fun `should return only the migration requested`() {
      ...
      webTestClient.get().uri("/mapping/allocations/migration/migration-id/$MIGRATION_ID")
        ...
        .jsonPath("content[0].activityScheduleId").isEqualTo(ACTIVITY_SCHEDULE_ID)
        ...
        .jsonPath("content[1].activityScheduleId").isEqualTo(ACTIVITY_SCHEDULE_ID + 1)
