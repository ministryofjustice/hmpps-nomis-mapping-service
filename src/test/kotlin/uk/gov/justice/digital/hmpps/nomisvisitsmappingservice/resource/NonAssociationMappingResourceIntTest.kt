        ...
        .body(
          BodyInserters.fromValue(
            createNonAssociationMapping(
              nomisBookingId = 1,
              nomisTypeSequence = 1,
              nonAssociationServiceId = 1,
              label = "2022-01-02T10:00:00",
              mappingType = NonAssociationMappingType.MIGRATED.name,
            ),
          ),
        )
        .exchange()
        ...
        .body(
          BodyInserters.fromValue(
            createNonAssociationMapping(
              nomisBookingId = 99,
              nomisTypeSequence = 2,
              nonAssociationServiceId = 199,
              label = "whatever",
              mappingType = NonAssociationMappingType.NOMIS_CREATED.name,
            ),
          ),
        )
        .exchange()
...
/*
  ...
  inner class DeleteMappingsTest {
    ...
    fun `delete mapping success`() {
      ...
      webTestClient.get().uri("/mapping/non-associations/nomis-booking-id/$bookingId/nomis-nonAssociation-sequence/$sequence")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_NON_ASSOCIATIONS")))
        .exchange()
        .expectStatus().isOk
    ...
    fun `delete nonAssociation mappings - migrated mappings only`() {
      ...
      webTestClient.get().uri("/mapping/non-associations/nonAssociation-id/222")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_NON_ASSOCIATIONS")))
        .exchange()
        .expectStatus().isNotFound
    }
  ...
  @DisplayName("DELETE /mapping/non-associations/nonAssociation-id/{nonAssociationId}")
  @Nested
  inner class DeleteMappingTest {

    @Test
    fun `access forbidden when no authority`() {
      webTestClient.delete().uri("/mapping/non-associations/nonAssociation-id/999")
        .exchange()
        .expectStatus().isUnauthorized
    }
    ...
    fun `delete specific mapping success`() {
      ...
      webTestClient.get().uri("/mapping/non-associations/nonAssociation-id/$nonAssociationId")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_NON_ASSOCIATIONS")))
        .exchange()
        .expectStatus().isOk
      ...
      // delete mapping
      webTestClient.delete().uri("/mapping/non-associations/nonAssociation-id/$nonAssociationId")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_NON_ASSOCIATIONS")))
        .exchange()
        .expectStatus().isNoContent
