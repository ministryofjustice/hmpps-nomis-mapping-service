...
import uk.gov.justice.digital.hmpps.nomismappingservice.incidents.IncidentMappingType.DPS_CREATED
import uk.gov.justice.digital.hmpps.nomismappingservice.incidents.IncidentMappingType.MIGRATED
import uk.gov.justice.digital.hmpps.nomismappingservice.incidents.IncidentMappingType.NOMIS_CREATED
...
class IncidentMappingResourceIntTest : IntegrationTestBase() {
  ...
  @Nested
  inner class GetIncidentMappingByNomisId {
  ...
  @Nested
  inner class GetIncidentMappingByDpsId {
  ...
  inner class GetMappingsByNomisIds {
    ...
    fun setUp() = runTest {
      mapping1 = repository.save(
        IncidentMapping(
          dpsIncidentId = UUID.randomUUID().toString(),
          nomisIncidentId = 54321,
    ...
    inner class HappyPath {
      ...
      fun `will return 200 when mapping does exist`() {
        webTestClient.get()
          ...
          .expectStatus().isOk
          .expectBody()
          .jsonPath("[0].nomisIncidentId").isEqualTo(mapping1.nomisIncidentId)
          .jsonPath("[0].dpsIncidentId").isEqualTo(mapping1.dpsIncidentId)
          ...
          .jsonPath("[1].nomisIncidentId").isEqualTo(mapping2.nomisIncidentId)
          .jsonPath("[1].dpsIncidentId").isEqualTo(mapping2.dpsIncidentId)
