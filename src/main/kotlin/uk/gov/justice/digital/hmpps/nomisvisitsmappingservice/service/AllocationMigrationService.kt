...
class AllocationMigrationService(
    ...
    with(createMappingRequest) {
      allocationMigrationMappingRepository.findById(nomisAllocationId)?.run {
        if (this@run.activityAllocationId == this@with.activityAllocationId) {
          log.debug("Allocation mapping already exists for allocation Ids: $activityAllocationId and nomisAllocationId: $nomisAllocationId so not creating. All OK")
          return
        }
        throw ValidationException("Nomis allocation mapping id = $nomisAllocationId already exists")
      }

      allocationMigrationMappingRepository.findOneByActivityAllocationId(
        activityAllocationId = activityAllocationId,
      )?.run {
        throw ValidationException("Allocation migration mapping with DPS allocation id=$activityAllocationId already exists")
      }
      ...
      telemetryClient.trackEvent(
        "allocation-migration-mapping-created",
        mapOf(
  ...
  suspend fun getMapping(allocationId: Long): AllocationMigrationMappingDto =
    allocationMigrationMappingRepository.findById(allocationId)
      ?.let { AllocationMigrationMappingDto(it) }
      ?: throw NotFoundException("nomisAllocationId=$allocationId")
