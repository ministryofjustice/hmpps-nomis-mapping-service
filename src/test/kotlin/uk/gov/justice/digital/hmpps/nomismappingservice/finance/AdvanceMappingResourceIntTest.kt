...
class AdvanceMappingResourceIntTest : IntegrationTestBase() {
  @Autowired
  private lateinit var repository: AdvanceMappingRepository

  @Nested
  @DisplayName("GET /mapping/advances/nomis-id/{nomisAdvanceId}")
  inner class GetMappingByNomisId {
    lateinit var mapping: AdvanceMapping
    ...
    inner class Security {
      ...
      fun `access not authorised when no authority`() {
        webTestClient.get()
          .uri("/mapping/advances/nomis-id/${mapping.nomisId}")
          .exchange()
    ...
    inner class HappyPath {
      ...
      fun `will return 404 when mapping does not exist`() {
        webTestClient.get()
          .uri("/mapping/advances/nomis-id/9999")
      ...
      fun `will return 200 when mapping does exist`() {
        webTestClient.get()
          ...
          .jsonPath("nomisAdvanceId").isEqualTo(mapping.nomisId)
  ...
  @Nested
  @DisplayName("GET /mapping/advances/dps-id/{dpsId}")
  inner class GetMappingByDpsId {
    ...
    inner class Security {
      ...
      fun `access not authorised when no authority`() {
        webTestClient.get()
          .uri("/mapping/advances/dps-id/${mapping.dpsId}")
          .exchange()
          .expectStatus().isUnauthorized
      }
    ...
    inner class HappyPath {
      ...
      fun `will return 404 when mapping does not exist`() {
        webTestClient.get()
          .uri("/mapping/advances/dps-id/DOESNOTEXIST")
          .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_MAPPING_API__SYNCHRONISATION__RW")))
          .exchange()
          .expectStatus().isNotFound
      }
  ...
  @Nested
  @DisplayName("DELETE /mapping/advances/dps-id/{dpsId}")
  ...
  @Nested
  @DisplayName("POST /mapping/advances")
  inner class CreateMapping {
    private lateinit var existingMapping: AdvanceMapping
    private val mapping = AdvanceMappingDto(
      dpsId = "A1234KU",
      nomisAdvanceId = 12346,
      label = "2023-01-01T12:45:12",
      mappingType = MIGRATED,
    )
      ...
      fun `returns 201 when mapping created`() = runTest {
        ...
        val createdMapping =
          repository.findOneByNomisId(
            nomisId = mapping.nomisAdvanceId,
          )!!
      ...
      fun `returns 409 if nomis id already exists`() {
        ...
        val duplicateResponse = webTestClient.post()
          ...
          .body(
            BodyInserters.fromValue(
              AdvanceMappingDto(
                dpsId = dpsId,
                nomisAdvanceId = existingMapping.nomisId,
              ),
            ),
          )
          .exchange()
        ...
        with(duplicateResponse!!) {
          assertThat(this.moreInfo.existing)
            .containsEntry("nomisAdvanceId", existingMapping.nomisId.toInt())
  ...
  @Nested
  @DisplayName("DELETE /mapping/advances")
  inner class DeleteAllMappings {
    private lateinit var existingMapping1: AdvanceMapping
    private lateinit var existingMapping2: AdvanceMapping
  ...
  @DisplayName("GET /mapping/advances/migration-id/{migrationId}")
  @Nested
  inner class GetMappingByMigrationId {
    ...
    inner class Security {
      @Test
      fun `access not authorised when no authority`() {
        webTestClient.get().uri("/mapping/advances/migration-id/2022-01-01T00:00:00")
          .exchange()
          .expectStatus().isUnauthorized
      }
    ...
    fun `can retrieve all mappings by migration Id`() = runTest {
      ...
      webTestClient.get().uri("/mapping/advances/migration-id/2023-01-01T12:45:12")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_MAPPING_API__SYNCHRONISATION__RW")))
        .exchange()
        .expectStatus().isOk
        .expectBody()
        .jsonPath("page.totalElements").isEqualTo(4)
        .jsonPath("$.content..nomisAdvanceId")
    ...
    fun `200 response even when no mappings are found`() {
      webTestClient.get().uri("/mapping/advances/migration-id/2044-01-01")
        .headers(setAuthorisation(roles = listOf("ROLE_NOMIS_MAPPING_API__SYNCHRONISATION__RW")))
        .exchange()
        .expectStatus().isOk
        .expectBody()
    ...
    fun `can request a different page size`() = runTest {
      ...
      webTestClient.get().uri {
        it.path("/mapping/advances/migration-id/2023-01-01T12:45:12")
          .queryParam("size", "2")
