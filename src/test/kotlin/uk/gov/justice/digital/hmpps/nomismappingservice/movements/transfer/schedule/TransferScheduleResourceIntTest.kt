...
class TransferScheduleResourceIntTest(
  ...
  @Nested
  @DisplayName("DELETE /mapping/transfer-scheduler/schedule/nomis-id/{nomisEventId}")
  inner class DeleteTransferScheduleMappingByNomisId {
    ...
    inner class HappyPath {
      ...
      @Test
      fun `should delete transfer schedule mapping by NOMIS ID`() = runTest {
        scheduleRepository.save(mapping1)
        scheduleRepository.save(mapping2)

        webTestClient.deleteTransferScheduleMapping(mapping1.nomisEventId)
    ...
    inner class Validation {
      ...
      fun `delete endpoint should be idempotent`() = runTest {
        webTestClient.deleteTransferScheduleMapping(12345L)
          .expectStatus().isNoContent
      }
    }
    ...
    private fun WebTestClient.deleteTransferScheduleMapping(nomisEventId: Long) = delete()
